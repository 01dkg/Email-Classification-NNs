...
def runTest(train, test, learner, totalClassifications=None, writeConfusion=False):
    ...
    print
    'Start time %s, End time %s, Dif %s' % (str(start), str(end), str(end - start))
...
def kFoldCrossValidation(data, learner, returnBest=False, folds=10):
    ...
    print
    'Average success train %f, test %f' % (averageSuccessTrain, averageSuccessTest)
    print
    '----------------------------------------------------------'
    if returnBest:
...
def testTreeLearning(data, test=None, getBest=False, folds=10, writeConfusion=False, **kw):
    print
    'Starting Decision Tree learning'
...
def getTreeData(data, testData=None, fileName='StatsTree', pruneDepth=False):
    if pruneDepth:
        ...
        getDataWrapper(maxes, fileName,
                       lambda m: testTreeLearning(data, test=testData, max_depth=m, same_majority_pruning=True))
    else:
        maxes = ['Max majority'] + [i * 0.05 for i in range(4, 21)]
        getDataWrapper(maxes, fileName,
                       lambda m: testTreeLearning(data, teste=testData, max_majority=m, same_majority_pruning=True))
